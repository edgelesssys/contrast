...
for file in "${!files[@]}"; do
  ...
  for ((i = 0; i < platform_count; i++)); do
    ...
    for test in "${tests[@]}"; do
      excluded=$(yq -o=json ".$MATRIX_PATH.exclude[]?" "$MATRIX_FILE" |
        jq -e --argjson sh "$self_hosted" --arg t "$test" 'select(."test-name" == $t and .platform."self-hosted" == $sh)' >/dev/null && echo true || echo false)
