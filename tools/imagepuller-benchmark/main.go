...
func run(cmd *cobra.Command, args []string) error {
	...
	f, err := os.CreateTemp("", "imagepuller-config-*.toml")
	if err != nil {
		return err
	}
	configFile := f.Name()
...
insecure-skip-verify = true
	...
	results := map[string]resourceUsage{}
	resultsIndividual, err := profileServerIndividual(imageList, binPath, imagepullerDir, "imagepuller", "--storepath", imagepullerDir, "--config", configFile)
	if err != nil {
		return err
	}
	maps.Copy(results, resultsIndividual)
	resultsContinuous, err := profileServerContinuous(imageList, binPath, imagepullerDir, "imagepuller", "--storepath", imagepullerDir, "--config", configFile)
	if err != nil {
		return err
	}
