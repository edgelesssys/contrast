...
func startServerWithMemoryTracking(ctx context.Context, serverPath string, args ...string) (func() (int, error), int, error) {
	...
	timeCmd := exec.CommandContext(ctx, "which", "time")
	out, err := timeCmd.Output()

	var exitErr *exec.ExitError
	if errors.As(err, &exitErr) {
		return nil, 0, fmt.Errorf("which returned non-zero exit code, stderr is: %s", string(exitErr.Stderr))
	} else if err != nil {
		return nil, 0, fmt.Errorf("failed to execute which: %w", err)
	}

	timePath := strings.TrimSpace(string(out))

	argsFull := append([]string{"-v", serverPath}, args...)
	cmd = exec.CommandContext(ctx, timePath, argsFull...)
	cmd.Stdout = nil
	cmd.Stderr = &stderr
