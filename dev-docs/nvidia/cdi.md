...
When adding a model-specific GPU resource name, update the mapping in the [GPU E2E test](../../e2e/gpu/gpu_test.go), the discovery list in the [`justfile`](../../justfile), and both the [development](../../packages/by-name/kata/genpolicy/genpolicy_settings_dev.patch) and [production](../../packages/by-name/kata/genpolicy/genpolicy_settings_prod.patch) genpolicy settings patches.
