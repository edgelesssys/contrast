...
## Adding support for a GPU model

When adding a model-specific GPU resource name, update the mapping in the [GPU end-to-end test](../../e2e/gpu/gpu_test.go), the discovery list in the [root recipe file](../../justfile), and both the [development](../../packages/by-name/kata/genpolicy/genpolicy_settings_dev.patch) and [production](../../packages/by-name/kata/genpolicy/genpolicy_settings_prod.patch) genpolicy settings patches.
Regenerate the genpolicy patches with the workflow documented in their [`package.nix`](../../packages/by-name/kata/genpolicy/package.nix).
