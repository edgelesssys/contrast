...
## Packets and updates (SNP)

AMD SEV-SNP is supported by kernels newer than 6.11 (see https://www.phoronix.com/news/Linux-6.11-KVM).
...
```bash
curl -fsL https://github.com/derailed/k9s/releases/latest/download/k9s_Linux_amd64.tar.gz | tar -C /usr/local/bin -xz k9s
```

Install helm
```bash
curl -fsL https://get.helm.sh/helm-v3.17.1-linux-amd64.tar.gz | tar -C /usr/local/bin -xz linux-amd64/helm
```
...
and exectue the given script on the machine.
...
Note that those instructions have to be executed every time the server is restarted.
```bash
...
installation. If the paths do not exist, then copy over your PATH into
...
i.e. "tdx" for TDX servers and "snp" for SNP servers.
