...
🔗 For details, vist the [Contrast documentation](https://docs.edgeless.systems/contrast/basics/security-benefits#personas-in-a-contrast-deployment).
...
- ✅ Unexpected environment variables cannot be injected.
...
- **ReferenceValues**: The launch digests of the CVMs, based on AMD SEV-SNP. This does not include any application code but tracks the setup of the CVM. Two pods can have (and typically have) the same CVM setup.
...
Whether mTLS connections based on service-mesh certificates should be used for incoming and outcoming traffic of pods can be configured by using annotations in the deployment YAML.
...
Managed Kubernetes like AKS offers some CoCo-enabled node pools with nested virtualization, as nodes in case of AKS are not bare-metal machines but already virtualized. See the [Contrast documentation](https://docs.edgeless.systems/contrast/getting-started/cluster-setup) for details.
...
When an uninitialized node (with only a basic node OS installed) joins the cluster (e.g., via K3s), K3s automatically installs:
...
3. Deploys the (Initial Guest Virtual Machine) IGVM file. The file includes:
   ...
   - Initramfs: Minmal filesystem used during early boot
...
- The runtime plugin instructs `QEMU` to boot the CVM with the IGVM file as the inital memory content.
...
- `QEMU` injects `HOSTDATA` into the CVM through the `SEV-SNP LAUNCH_SECRET` mechanism, securely storing it within AMD’s hardware-protected memory region.
  ...
  - Although `HOSTDATA` is not included in the initial memory image, it is always embedded and cryptographically signed as part of the attestation report.
...
- During pod initialization, the runtime shim invokes the `SetPolicy` function of the `kata-agent`, which performs the following actions:
  ...
  - If the hashes do not match, the CVM immediately aborts execution to prevent policy violations.
...
✅ These steps ensure that only attested workloads can join the trusted service mesh. By relying exclusively on this service mesh for communication, workloads and data owners have assurance that they are interacting solely with trusted services defined by the configured manifest.
...
- The Contrast CLI is distributed with embedded reference hashes (e.g., launch digest and policy hash) for the Contrast coordinator.
...
- By initiating the manifest upload throug the `set` command, The CLI initiates an aTLS connection to the Contrast coordinator.
...
- **Root filesystem (rootfs)** – Verified indirectly since the kernel includes `dm-verity` drivers, and the root filesystem’s reference hash is provided as part of the kernel command line.
...
### What is derived from the recovery seed?

- **Manifest Signing Key**: Used to ensure integrity and authenticity of the manifest stored on untrusted persistent volumes.
- **Workload Secret Seeds**: Distributed securely to attested workloads, enabling them to derive encryption keys for securely storing sensitive data (e.g., secret keys) on untrusted persistent volumes.
