...
func Coordinator(namespace string) *CoordinatorConfig {
	c := StatefulSet("coordinator", namespace).
		WithSpec(StatefulSetSpec().
				...
				WithSpec(PodSpec().
					...
					WithVolumes(
						Volume().
							...
							WithPersistentVolumeClaim(applycorev1.PersistentVolumeClaimVolumeSource().WithClaimName("image-store")),
					).
					WithContainers(
						Container().
			...
			WithVolumeClaimTemplates(PersistentVolumeClaim("image-store", "").
