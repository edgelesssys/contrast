...
// BinaryMarshal marshals the ID block to binary.
func (idBlock *IDBlock) BinaryMarshal() ([]byte, error) {
	data := make([]byte, 0x60)
...
// BinaryUnmarshal unmarshals the ID block from binary.
func (idBlock *IDBlock) BinaryUnmarshal(data []byte) error {
	if len(data) != 0x60 {
...
// BinaryMarshal marshals the ID authentication block to binary.
func (idAuth *IDAuthentication) BinaryMarshal() ([]byte, error) {
...
// BinaryUnmarshal unmarshals the ID authentication block from binary.
func (idAuth *IDAuthentication) BinaryUnmarshal(data []byte) error {
	if len(data) != 0x1000 {
	...
	if err := idAuth.IDBlockSig.BinaryUnmarshal(data[0x40:0x240]); err != nil {
		return fmt.Errorf("failed to unmarshal ID block signature: %w", err)
	}

	if err := idAuth.IDKey.BinaryUnmarshal(data[0x240:0x644]); err != nil {
		return fmt.Errorf("failed to unmarshal ID key: %w", err)
	}

	copy(idAuth.Reserved1[:], data[0x644:0x680])

	if err := idAuth.IDKeySig.BinaryUnmarshal(data[0x680:0x880]); err != nil {
		return fmt.Errorf("failed to unmarshal ID key signature: %w", err)
	}

	if err := idAuth.AuthKey.BinaryUnmarshal(data[0x880:0xC84]); err != nil {
...
type Ecdsa384Sha384Signature struct {
	R         [0x48]byte
	S         [0x48]byte
	Reserved1 [0x200]byte
}

// BinaryMarshal marshals the ECDSA P-384 with SHA-384 signature to binary.
func (sig *Ecdsa384Sha384Signature) BinaryMarshal() ([]byte, error) {
...
// BinaryUnmarshal unmarshals the ECDSA P-384 with SHA-384 signature from binary.
func (sig *Ecdsa384Sha384Signature) BinaryUnmarshal(data []byte) error {
	if len(data) != 0x200 {
...
// BinaryMarshal marshals the ECDSA P-384 public key to binary.
func (pubKey *Ecdsa384PublicKey) BinaryMarshal() ([]byte, error) {
...
// BinaryUnmarshal unmarshals the ECDSA P-384 public key from binary.
func (pubKey *Ecdsa384PublicKey) BinaryUnmarshal(data []byte) error {
	if len(data) != 0x404 {
...
func IDBlocksFromLaunchDigest(launchDigest [48]byte) (*IDBlock, *IDAuthentication, error) {
	...
	idBlockBytes, err := idBlk.BinaryMarshal()
	if err != nil {
