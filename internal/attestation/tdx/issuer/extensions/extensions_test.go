...
func TestMarshalling(t *testing.T) {
	...
	var expectedQuote tdx.QuoteV4
	require.NoError(protojson.Unmarshal([]byte(quoteJSON), &expectedQuote))

	expectedExtensions := []*Extension{
		{Collateral: []byte{0x42}},
	}

	data, err := Marshal(&expectedQuote, expectedExtensions...)
	require.NoError(err)
	quote, extensions, err := Unmarshal(data)
	require.NoError(err)

	protoEqual(t, &expectedQuote, quote)
	require.Equal(len(expectedExtensions), len(extensions))

	for i := range len(expectedExtensions) {
		protoEqual(t, expectedExtensions[i], extensions[i], "extension %d", i)
	}
}

func protoEqual(t *testing.T, expected, actual proto.Message, msgAndArgs ...any) {
	if proto.Equal(expected, actual) {
		return
	}

	opts := &protojson.MarshalOptions{Multiline: true}
	e, err := opts.Marshal(expected)
	require.NoError(t, err)
	a, err := opts.Marshal(actual)
	require.NoError(t, err)
