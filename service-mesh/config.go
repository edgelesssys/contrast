...
func ParseProxyConfig(ingressConfig, egressConfig string) (ProxyConfig, error) {
	...
	for _, entry := range strings.Split(ingressConfig, "##") {
		...
		disableClientCertificate, err := strconv.ParseBool(parts[2])
		if err != nil {
			return ProxyConfig{}, fmt.Errorf("invalid disable client certificate: %s", parts[2])
		}
		cfg.ingress = append(cfg.ingress, ingressConfigEntry{
			name:                     parts[0],
			listenPort:               uint16(listenPort),
			disableClientCertificate: disableClientCertificate,
		})
