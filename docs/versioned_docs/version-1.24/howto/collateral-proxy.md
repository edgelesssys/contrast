...
Both the [Coordinator](../architecture/components/coordinator.md) and the [Initializer](../architecture/components/initializer.md) need to fetch this collateral to verify a peer.
...
No special considerations are required, since the proxy doesn't run in a confidential VM.
