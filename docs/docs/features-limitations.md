...
- **Image pull secrets**: registry authentication is only supported on AKS. On bare metal, you can work around this by mirroring the images to a local registry without authentication. Such a mirror registry needs to be reachable from the confidential VMs and have a valid TLS certificate.
