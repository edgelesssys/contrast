...
At the end of this document, we included an [FAQ](#frequently-asked-questions-about-attestation-in-contrast) that answers the most common questions regarding attestation in hindsight of the [security benefits](../../old/basics/security-benefits.md).
...
Their evidence is rooted in the [hardware measurements](../../old/basics/confidential-containers.md) from the CPU and their [confidential VM environment](../../old/components/runtime.md).
...
Pods run in Contrast's [runtime environment](../../old/components/runtime.md) (B), effectively within a confidential VM.
...
The root filesystem contains all components of the container's runtime environment including the [guest agent](../../old/basics/confidential-containers.md#kata-containers) (C).

In the userland, the guest agent takes care of enforcing the [runtime policy](../../old/components/overview.md#runtime-policies) of the pod.
...
During the [deployment](../../old/deployment.md#generate-policy-annotations-and-manifest) the policy is annotated to the Kubernetes Pod resources.
...
The [Coordinator](../../old/components/overview.md#the-coordinator) acts as a verifier within the Contrast deployment, configured with a [Manifest](../../old/components/overview.md#the-manifest) that defines the reference values and serves as an appraisal policy for all pods in the deployment.
...
The [CLI](../../old/components/overview.md#the-cli-command-line-interface) serves as the verifier for the Coordinator and the entire Contrast deployment, containing the reference values for the Coordinator and the endorsements from hardware vendors.
...
A relying party in the Contrast scenario could be, for example, the [data owner](../../old/basics/security-benefits.md) that interacts with the application.
The relying party can use the CLI to obtain the attestation results and Contrast's [CA certificates](../../old/architecture/certificates.md) bound to these results.
