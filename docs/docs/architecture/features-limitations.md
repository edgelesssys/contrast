...
- **Bare-metal support**: Support for running [Contrast on bare-metal Kubernetes](../howto/cluster-setup/bare-metal.md) is available for AMD SEV-SNP and Intel TDX.
    Availability in the public cloud is limited to cloud vendors providing the required hardware as bare-metal instances, ideally as part of their managed Kubernetes offerings.
