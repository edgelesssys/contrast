...
- **Isolation**: By design, Contrasts strictly isolates workloads from the rest of the infrastructure. Contrast excludes any access from the infrastructure provider, datacenter employees, priviliged cloud admins, and external malicious actors.
...
> **Out of scope:**
>
> - Vulnerabilities in application logic (e.g., broken access controls)
> - Hardware-level attacks on Confidential Computing (e.g., side-channel exploits)
...
- **Malicious cloud insider:**
  Cloud provider employees or contractors with privileged access across physical infrastructure, hypervisors, or Kubernetes control planes. They may tamper with VM resources, intercept data, or restrict runtime behavior (e.g., limit memory, alter disk volumes, or change firewall rules).
...
- **Malicious container image provider:**
  Publishes container images that include malicious functionality (e.g., embedded backdoors or unauthorized data access logic).
...
| Cloud insider, co-tenant, operator | Confidential Container, Workload | Container runtime        | May use APIs (e.g., `kubectl exec`) to access workloads                        |
...
| Reading or altering data on persistent volumes | Persistent volumes are not yet supported; future support will include encryption and integrity protections | [Runtime environment](./architecture/components/runtime.md) |
...
| Use Case                   | Example                                                                                                                                                                                                                  |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Secure cloud migration** | _TechSolve Inc._ moves sensitive workloads to the cloud. As both image provider and data owner, they are exposed to threats from insiders and co-tenants. Contrast ensures data isolation and workload integrity.        |
