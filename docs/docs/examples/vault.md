...
If no changes are needed to the deployment files, you can proceed directly to restarting the Vault pods.
However, if you do need to update the deployment, ensure that the `workloadSecretID` of the Vault remains unchanged.

:::note Optional: Updating the deployment before restart

When modifying the deployment configuration, you must regenerate the runtime policies and apply the new manifest using the following commands:
...
Once any optional updates have been applied—or if no changes were needed—you can restart the Vault using:
