...
By default, the images are pulled and uncompressed into an encrypted storage device using [Contrast secure mount](../secured-image-storage.md).
...
Altogether, setting the limit to 10x the compressed image size should be sufficient for small to medium images when not using the Contrast secure mount feature.
