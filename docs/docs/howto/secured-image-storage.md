...
metadata:
  annotations:
    contrast.edgeless.systems/trusted-store-size: 250Gi
```
...
metadata:
  annotations:
    contrast.edgeless.systems/trusted-store-size: 0
```

To disable Contrast secure mount globally, specify the `--skip-trusted-store` flag in the `generate` command.
