...
func (s *StubStore) CreateImage(_ string, _ []string, _, _ string, _ *storage.ImageOptions) (*storage.Image, error) {
	panic("not implemented")
}

func (s *StubStore) CreateContainer(_ string, _ []string, _, _, _ string, _ *storage.ContainerOptions) (*storage.Container, error) {
	panic("not implemented")
}
...
func (s *StubStore) Mount(_, _ string) (string, error) {
	panic("not implemented")
}
...
func TestGetAndVerifyImage_EvilRegistry(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			_, err := s.getAndVerifyImage(context.Background(), log, fmt.Sprintf("%s/busybox:v0.0.1@%s", server.URL[7:], tc.digest))
