...
// An attacker with k8n admin privileges could thus change the config..
...
func (c *Config) registryFor(name string) Registry {
	...
	maxMatchingLabeld := -1 // "." has 0 matching labels.
	for fqdn, registryCandidate := range c.Registries {
		name = strings.TrimSuffix(name, ".")
		fqdn = strings.TrimSuffix(fqdn, ".")
		matchingLabels := strings.Count(fqdn, ".")
		if matchingLabels > maxMatchingLabeld && strings.HasSuffix(name, fqdn) {
			maxMatchingLabeld = matchingLabels
