...
func TestMemDump(t *testing.T) {
	...
	t.Run("memory dump does not contain canary string", func(t *testing.T) {
		...
		filterStr := strings.Join([]string{
			"labels.\\\"io.kubernetes.pod.namespace\\\"==" + ct.Namespace,
			"labels.\\\"io.kubernetes.pod.name\\\"==" + listenerPods[0].Name,
			"labels.\\\"io.cri-containerd.kind\\\"==sandbox",
		}, ",")
		argv = []string{
			"/bin/sh",
			"-c",
			"nsenter --target 1 --mount -- " + ctrCmd + " -n k8s.io c ls -q " + filterStr + " |" +
