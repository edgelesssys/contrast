...
func patchReferenceValues(k *kubeclient.Kubeclient, platform platforms.Platform) (PatchManifestFunc, error) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	tcbSpecs, err := k.GetConfigMapData(ctx, "default", "bm-tcb-specs", "tcb-specs.json")
	if err != nil {
		return nil, fmt.Errorf("Failed getting ConfigMap: bm-tcb-specs with error: %w", err)
	}
	err = json.Unmarshal(tcbSpecs, &baremetalRefVal)
	if err != nil {
		return nil, fmt.Errorf("Failed unmarshaling minimum tcb specification from ConfigMap: bm-tcb-specs with error: %w", err)
	}
