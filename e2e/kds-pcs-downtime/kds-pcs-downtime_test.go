...
func TestKDSPCSDowntime(t *testing.T) {
	...
	t.Run("kds downtime", func(t *testing.T) {
		if !strings.Contains(ct.RuntimeClassName, "snp") {
		...
		etcHosts, stderr, err := c.Exec(ctx, ct.Namespace, coordinatorPods[0].Name, []string{"/bin/sh", "-c", "cat /etc/hosts"})
		require.NoError(err, "stderr: %q", stderr)
		_, stderr, err = c.Exec(ctx, ct.Namespace, coordinatorPods[0].Name, []string{"/bin/sh", "-c", "echo 127.0.0.1 kdsintf.amd.com >> /etc/hosts"})
		require.NoError(err, "stderr: %q", stderr)
		...
		_, stderr, err = c.Exec(ctx, ct.Namespace, coordinatorPods[0].Name, []string{"/bin/sh", "-c", fmt.Sprintf("echo '%s' > /etc/hosts", etcHosts)})
	...
	t.Run("pcs downtime", func(t *testing.T) {
		if !strings.Contains(ct.RuntimeClassName, "tdx") {
