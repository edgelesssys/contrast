	...
	for _, dir := range dirs {
		t.Run(dir.Name(), func(t *testing.T) {
			...
			for _, file := range files {
				...
				unmarshalledYAML := make(map[string]any)
			...
			for _, resource := range intermediateResources {
				...
				case *applyappsv1.DeploymentApplyConfiguration:
					...
					t.Cleanup(func() {
						if err := cleanupResource(t.Context(), resource, *r.Name, ct); err != nil {
							t.Logf("failed to delete resource %s: %s", *r.Name, err)
						}
					})
...
func cleanupResource(ctx context.Context, resource any, name string, ct *contrasttest.ContrastTest) error {
