...
func TestGPU(t *testing.T) {
	...
	resources := kuberesource.GPU(gpuDeploymentName, deviceURI, 0)
	...
	if platform == platforms.MetalQEMUTDXGPU {
		resources = append(resources, kuberesource.GPU(gpuDeploymentName+"-1", deviceURI, 1)...)
	}
	...
	require.True(t, t.Run("wait for GPU deployments", func(t *testing.T) {
		...
		for _, deploymentName := range gpuDeployments {
	...
	}), "GPU deployments need to succeed for subsequent tests")
