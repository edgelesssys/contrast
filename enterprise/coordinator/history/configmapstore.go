...
func NewConfigMapStore(client kubernetes.Interface, log *slog.Logger) (*ConfigMapStore, error) {
	...
	defer cancel()
	self, err := client.CoreV1().Pods(string(namespace)).Get(ctx, os.Getenv("HOSTNAME"), metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
...
func (s *ConfigMapStore) Has(key string) (bool, error) {
	...
	_, err := s.client.CoreV1().ConfigMaps(s.namespace).Get(ctx, objectName(key), metav1.GetOptions{})
...
func (s *ConfigMapStore) Watch(key string) (<-chan []byte, func(), error) {
	...
	watcher, err := s.client.CoreV1().ConfigMaps(s.namespace).Watch(context.Background(), metav1.ListOptions{
		...
		FieldSelector: fields.OneTermEqualSelector("metadata.name", objectName(key)).String(),
	})
	if err != nil {
		return nil, nil, err
	}
...
func (s *ConfigMapStore) newEntry(key string, value []byte) *corev1.ConfigMap {
...
func objectName(key string) string {
	return fmt.Sprintf("contrast-store-%s", strings.ReplaceAll(key, "/", "-"))
}
