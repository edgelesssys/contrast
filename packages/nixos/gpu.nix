...

{
  ...
  config = lib.mkIf config.contrast.gpu.enable {
    hardware.nvidia = {
      open = true;
      package = pkgs.contrastPkgs.nvidia-driver;
    ...
    hardware.graphics.package = pkgs.contrastPkgs.nvidia-driver;
    hardware.graphics.package32 = pkgs.contrastPkgs.nvidia-driver;
