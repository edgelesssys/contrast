...
sandbox=$(crictl --config /dev/null --runtime-endpoint /run/containerd/containerd.sock inspectp |
  jq '.[]' |
...
trap 'rm -f "$key"' EXIT
rm "$key"
ssh-keygen -t ed25519 -f "$key" -C "" -N ""
ssh -o "ProxyCommand=socat - VSOCK-CONNECT:$cid:22" -o StrictHostKeyChecking=false -i "$key" root@localhost "$@"
