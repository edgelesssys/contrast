...
func TestSetManifestHandler(t *testing.T) {
	validRequest := &apitypesv1.SetManifestRequest{
	...
	testCases := map[string]struct {
		...
		"more than one signature": {
			...
			expStatus: http.StatusBadRequest,
			expErr:    errTooManySignatures.Error(),
		},
		"invalid manifest": {
			request:   validRequest,
			...
			expCode:   apitypes.ErrorCodeInvalidArgument,
		},
		"unauthorized": {
			request:   validRequest,
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			case tc.expStatus != http.StatusOK:
				...
				if tc.expCode != "" {
					assert.Equal(tc.expCode, apiErr.Code)
				}
			default:
				...
				assert.Equal(tc.request.Signatures[0], tc.setter.gotReq.GetSignature())
...
func TestSetManifestHandlerNoSeedShares(t *testing.T) {
	...
	bodyBytes, err := json.Marshal(&apitypesv1.SetManifestRequest{Manifest: []byte("{}")})
	require.NoError(err)
	req := httptest.NewRequestWithContext(t.Context(), http.MethodPost, "/v1/manifest", bytes.NewReader(bodyBytes))
