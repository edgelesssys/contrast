...
func TestBadStoreWatcherIsRestarted(t *testing.T) {
	...
	clock := &waitingClock{
		...
		newTimerCalls: make(chan struct{}, 1),
	}
	a.clock = clock
	...
	clock.WaitForNewTimerCall(t, time.Second)
...
type waitingClock struct {
	*testingclock.FakeClock
	newTimerCalls chan struct{}
}

// NewTimer is overridden so that we know when a timer was created. Otherwise, we might be stepping
// the clock before something is waiting on it.
func (c *waitingClock) NewTimer(d time.Duration) clock.Timer {
	t := c.FakeClock.NewTimer(d)
	c.newTimerCalls <- struct{}{}
	return t
}

func (c *waitingClock) WaitForNewTimerCall(t *testing.T, d time.Duration) {
	select {
	case <-time.After(d):
		require.Fail(t, "no call to NewTimer")
	case <-c.newTimerCalls:
	}
}
